...
### 1/1  
서버 부분 작업 시작. 서버 역시 JFrame으로 작업. Client에서 보내는 내용이 정상적으로 TextArea에 표시되는 것 확인. 이제 컴퓨터별로 번호를 부여하는 법이랑 여러개 컴퓨터로 동시 접속을 했을 때 작동이 잘 되는지, 그리고 exe화하는법 등만 해결하면 1차적으로는 완성이 될 것 같다.
